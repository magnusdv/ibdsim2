...
As of version 2.1.0, a local version of the Shiny app is also available within the `ibdsim2` R package. To ensure the app has all required dependencies, use the following installation command: 
```{r, eval=FALSE}
...
The most important function in **ibdsim2** is `ibdsim()`, which simulates the recombination process in a given pedigree. In this example we demonstrate this in a family quartet, and show how to visualise the result.
...
For simplicity we create a pedigree containing all three relationships we are interested in.
